...
/**
 * Processes an Excel file to extract vendors.
...
/**
 * Creates vendors in the database based on the provided data.
 * @param {any[]} itemsData - An array of vendorsdata.
 ...
 */
const createVendors = async (itemsData: any) => {
  let succeed = 0
  let failed = 0
  try {
    for (const itemData of itemsData) {
      ...
      const vendor = new Vendor(itemData)
      await vendor.save()
      succeed++
    }
    return {
...
/**
 * Handles API requests for uploading and processing an Excel file to create vendors.
  ...
  if (req.method === 'POST') {
    try {
      ...
      form.parse(req, async (err: any, fields: any, files: any) => {
        ...
        // Process the Excel file to extract vendors
        const inventoryItems = await processExcelFile(fileBuffer)

        // Create vendors in the database
        const createdItems = await createVendors(inventoryItems)
